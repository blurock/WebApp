...
public class DataInput extends Composite implements HasText {
	...
	private static DataInputUiBinder uiBinder = GWT
			.create(DataInputUiBinder.class);
	...
	//ClientDataInputToDatabase servercall = new ClientDataInputToDatabase();
	...
	public DataInput(String type,
			String title, String titletooltip,
				String httptext,
				String texttitle,
				String texttext) {
		initWidget(uiBinder.createAndBindUi(this));
	...
	private void init() {
		...
		SingleUploader uploader = new SingleUploader(
				FileInputType.CUSTOM.with(button), new MyUploadStatus());
	...
	void onTextUpload(ClickEvent e) {
		...
		//servercall.textToDatabase(uploadTextName.getText(),textarea.getText(), this);
	...
	void onHTTPUpload(ClickEvent e) {
		TextToDatabaseAsync async = TextToDatabase.Util.getInstance();
		async.httpToDatabase(httpaddress.getText(), callbackHTTP);
		//servercall.httpToDatabase(httpaddress.getText(), this);
